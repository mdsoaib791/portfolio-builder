          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {session?.user?.id &&
              <a
                ...
                onClick={() => setActive(`portfolio/${session?.user?.id}`)}
                className={`relative text-sm font-medium transition-colors ${active === `/portfolio/${session?.user?.id}`
                  ? 'text-blue-600'
                  : 'text-foreground/80 hover:text-foreground'
                  }`}
              >
                My Portfolio
                {active === `/portfolio/${session?.user?.id}` && (
                  <span className="absolute -bottom-1 left-0 w-full h-0.5 bg-blue-600 rounded"></span>
                )}
              </a>
            }
